...
public:
    ...
    // #4 cache-layout profiling: flag components carrying heap-indirection handles
    // (DynamicVector / DynamicString) so iteration can account for the miss.
    void markHeapHandles(ComponentTypeID typeID) {
        if (typeID < MAX_COMPONENT_TYPES) m_heapHandles[typeID] = true;
    }
    bool hasHeapHandles(ComponentTypeID typeID) const {
        if (typeID >= MAX_COMPONENT_TYPES) return false;
        return m_heapHandles[typeID];
    }

private:
    ComponentSizeRegistry() {
        m_registered.fill(false);
        m_heapHandles.fill(false);
    }
    
    std::array<size_t, MAX_COMPONENT_TYPES> m_sizes;
    std::array<bool, MAX_COMPONENT_TYPES> m_registered;
    // #4 cache-layout profiling: per-type flag for components whose in-struct fields
    // are heap-indirection handles (DynamicVector/DynamicString) -> cache miss on scan.
    std::array<bool, MAX_COMPONENT_TYPES> m_heapHandles;
};
...
// #4 cache-layout profiling: register a component type AND flag it as carrying
// heap-indirection handles (DynamicVector/DynamicString) in its fields.
template<typename T>
struct ComponentSizeRegistrarHeap {
    ComponentSizeRegistrarHeap(ComponentTypeID typeID) {
        ComponentSizeRegistry::instance().registerType<T>(typeID);
        ComponentSizeRegistry::instance().markHeapHandles(typeID);
    }
};
...
// #4 cache-layout profiling: register component T and flag it as carrying
// heap-indirection handles (DynamicVector/DynamicString). Use in place of
// ECS_REGISTER_COMPONENT for component types whose in-struct fields dereference a DynamicPool.
#define ECS_REGISTER_COMPONENT_HEAP(T) \
    static ::ecs::detail::ComponentSizeRegistrarHeap<T> ECS_CONCAT(_registrar_heap_, __LINE__)(::ecs::getComponentTypeID<T>())
