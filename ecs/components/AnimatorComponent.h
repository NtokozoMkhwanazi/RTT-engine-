...
#include "../BoneSoA.h"      // #L1075: member-split SoA bone storage for SkeletonComponent
#include <glm/glm.hpp>
...
// Guard: ecs::Bone name buffer (MAX_BONE_NAME_LENGTH) must match BoneSoA's
// fixed-name stream (BONE_NAME_LEN), otherwise bone names truncate silently.
static_assert(MAX_BONE_NAME_LENGTH == BoneSoA::BONE_NAME_LEN,
              "MAX_BONE_NAME_LENGTH must equal ecs::BoneSoA::BONE_NAME_LEN");
...
struct SkeletonComponent : public Component {
    BoneSoA bones;            // #L1075: member-split SoA (was DynamicVector<Bone> slab)
    int rootBoneIndex = -1;
...
struct AnimatorComponent : public Component {
    StaticVector<Animation*, 64> animations;  // #4 Step B: inline SmallBuffer (no slab hop)
    ...
    int activeLayer = 0;
    StaticVector<int, 16> animationLayers;  // #4 Step B: inline SmallBuffer (no slab hop)
    ...
    void setPool(DynamicPool*) {}  // #4 Step B: animations/animationLayers are inline (StaticVector)
