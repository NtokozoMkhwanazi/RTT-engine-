...
public:
    ...
    /**
     * Set the world pointer for iteration (archetype storage - editor light
     * entities are created via createEntityWithComponents, which the legacy
     * ComponentManager-based forEach() cannot see).
     */
    void setWorld(World* world) { m_world = world; }
    World* getWorld() const { return m_world; }
    ...
    void update(float deltaTime) override {
        ...
        m_world->forEach<TransformComponent, LightComponent>(
