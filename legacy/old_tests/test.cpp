...
// ================= ANIMATION FSM =================
// Using fixed FSM for animation management
// Fixed issues: jump exit, blend space, root motion, layer cleanup

AnimationStateMachine* fsm = nullptr;

// ================= HYBRID MM+FSM =================
// Hybrid Motion Matching + Finite State Machine system
...
void runCameraTests() {
    ...
    testCam.ProcessMouseMovement(400.0f, 300.0f);  // First call initializes mouse position
    testCam.ProcessMouseMovement(450.0f, 350.0f);  // Second call actually moves
...
// ============================================================================
// Find flat ground for model placement
// ============================================================================
glm::vec3 findFlatGround(Terrain* terrain, float minX, float maxX, float maxSearchHeight) {
    std::random_device rd;
    std::mt19937 gen(42);  // Fixed seed for reproducibility
    std::uniform_real_distribution<> disX(minX, maxX);
    std::uniform_real_distribution<> disZ(-50.0f, 50.0f);
    
    // Try to find position with height < maxSearchHeight
    float bestX = 0.0f, bestZ = 0.0f, bestHeight = 999999.0f;
    ...
    for (int i = 0; i < 50; i++) {
        ...
        // Keep track of lowest height found
        ...
        // If we found low ground, use it
        if (height < maxSearchHeight) {
            return glm::vec3(x, height, z);
        }
    }
    
    // Return lowest position found, even if not ideal
    std::cout << "[findFlatGround] Best found: (" << bestX << ", " << bestHeight << ", " << bestZ << ")\n";
...
int main()
{
    ...
    // Load multiple FBX models using ModelManager
    // NOTE: Only Bot is used as playable character
    // Other models are static props (rocks, decorations, etc.)
    modelManager.load("Bot", "assets/bot.fbx");              // PLAYABLE character
    modelManager.load("Bear", "assets/World_objects/Bear_DEMO.fbx");  // Static prop
    modelManager.load("Datsun", "assets/datsun.fbx");        // Static prop (car)
    modelManager.load("Rock1", "assets/World_objects/Rock1.fbx");      // World rock
    modelManager.load("Rock2", "assets/World_objects/Rock2.fbx");      // World rock
    modelManager.load("Stone", "assets/World_objects/stone.fbx");      // World stone

    modelManager.printAll();

    // Get initial character model (Bot - ONLY animated character)
    ...
    std::cout << "\n=== Character Skeleton ===\n";
    std::cout << "Skeleton bones: " << skeleton.bones.size() << " (ANIMATED)\n";
    std::cout << "Note: Bot is the ONLY playable character\n";
    std::cout << "Other models (Bear, Datsun) are static props\n";
    ...
    // Load all animations - using shared_ptr for proper lifetime management
    ...
    std::shared_ptr<Animation> fallAnim = nullptr;
    std::shared_ptr<Animation> crouchAnim = nullptr;
    std::shared_ptr<Animation> crouchWalkAnim = nullptr;
    ...
    // Helper lambda to load animation - returns shared_ptr with UNIQUE name
    auto loadAnim = [&](const std::string& path, const std::string& name) -> std::shared_ptr<Animation> {
        try {
            const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs);
            if (scene && scene->HasAnimations()) {
                // Create shared_ptr from raw pointer
                auto anim = std::shared_ptr<Animation>(
                    new Animation(AssimpAnimationLoader::LoadAnimationWithPoseCorrection(scene, scene->mAnimations[0]))
                );
                // FIX: Override the animation name to be unique (Mixamo sets all to "mixamo.com")
                anim->name = name;
                std::cout << "    [LOADED] " << name << ": " << anim->duration << "s, " 
                          << anim->boneAnimations.size() << " bones\n";
                return anim;
            }
            std::cerr << "    [FAILED] " << name << ": Could not load " << path << "\n";
            return nullptr;
    ...
    std::cout << "Idle: " << (idleAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "Walk: " << (walkAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "Run: " << (runAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "Jump: " << (jumpAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "Fall: " << (fallAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "Crouch: " << (crouchAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "CrouchWalk: " << (crouchWalkAnim ? "LOADED" : "NULL") << "\n";
    std::cout << "=============================\n\n";
    ...
    try {
        grassModel = new Model("assets/Bear_DEMO.fbx");
    ...
    std::cout << "  [FootIK] Initialized: L=" << leftFoot << " R=" << rightFoot << "\n";

    // FIX animation durations BEFORE loading into HybridMMFSM
    auto fixAnimationDuration = [](std::shared_ptr<Animation> anim, float expectedDuration, const std::string& name) {
        if (!anim) {
            std::cerr << "  [SKIP] " << name << ": NULL animation\n";
            return;
        }
        std::cout << "  [DEBUG] " << name << ": duration=" << anim->duration
                  << "s, bones=" << anim->boneAnimations.size() << "\n";
        fflush(stdout);  // Force flush to see where crash happens
        if (anim->duration > 5.0f) {  // Only fix if way too long
            anim->duration = expectedDuration;
            std::cout << "  [FIX] " << name << ": " << anim->name
                      << " duration TRUNCATED to " << anim->duration << "s\n";
            fflush(stdout);
        } else {
            std::cout << "  [OK] " << name << ": duration OK (" << anim->duration << "s)\n";
            fflush(stdout);
        }
    };
    
    std::cout << "\n=== FIXING ANIMATION DURATIONS ===\n";
    // Fix durations to reasonable values for gameplay
    // CRITICAL: Must be LONG ENOUGH to capture FULL gait cycle
    // A full gait cycle = left plant → right plant → left plant (next cycle)
    // Typical walk: ~1.2s per full cycle, run: ~0.8s per full cycle
    // We use slightly longer to ensure smooth transitions
    fixAnimationDuration(idleAnim, 2.5f, "Idle");    // 2.5s for full idle cycle
    fixAnimationDuration(walkAnim, 2.4f, "Walk");    // 2.4s = ~2 full gait cycles (1.2s each)
    fixAnimationDuration(runAnim, 1.6f, "Run");      // 1.6s = ~2 full gait cycles (0.8s each)
    fixAnimationDuration(jumpAnim, 1.1f, "Jump");    // 1.1s one-shot jump
    fixAnimationDuration(fallAnim, 1.0f, "Fall");    // 1.0s loop
    fixAnimationDuration(crouchAnim, 0.6f, "Crouch"); // 0.6s transition
    fixAnimationDuration(crouchWalkAnim, 1.8f, "CrouchWalk"); // 1.8s for full crouch gait cycle
    
    std::cout << "=== ANIMATION FIX COMPLETE ===\n";
    fflush(stdout);

    // Hybrid MM+FSM System
    std::cout << "  [DEBUG] Creating HybridMMFSM...\n";
    fflush(stdout);
    hybrid = new HybridMMFSM();
    std::cout << "  [DEBUG] HybridMMFSM created, initializing...\n";
    fflush(stdout);
    hybrid->Initialize(&skeleton, animator);
    std::cout << "  [DEBUG] HybridMMFSM initialized\n";
    fflush(stdout);
    showProgress(); std::cout << "Hybrid MM+FSM...\n";
    fflush(stdout);

    // Load locomotion animations (for Motion Matching) - skip NULL
    std::cout << "  [DEBUG] Loading Idle (ptr=" << (void*)idleAnim.get() << ")...\n"; fflush(stdout);
    if (idleAnim) {
        try {
            hybrid->LoadLocomotionAnimation("Idle", idleAnim);
            std::cout << "    Idle loaded: duration=" << idleAnim->duration << "s, bones=" << idleAnim->boneAnimations.size() << "\n";
        } catch (const std::exception& e) {
            std::cerr << "    [EXCEPTION in LoadLocomotionAnimation Idle]: " << e.what() << "\n";
            throw;
        }
    } else { std::cerr << "    [SKIP] idleAnim is NULL\n"; }
    std::cout << "  [DEBUG] Loading Walk (ptr=" << (void*)walkAnim.get() << ")...\n"; fflush(stdout);
    if (walkAnim) {
        try {
            hybrid->LoadLocomotionAnimation("Walk", walkAnim);
            std::cout << "    Walk loaded: duration=" << walkAnim->duration << "s, bones=" << walkAnim->boneAnimations.size() << "\n";
        } catch (const std::exception& e) {
            std::cerr << "    [EXCEPTION in LoadLocomotionAnimation Walk]: " << e.what() << "\n";
            throw;
        }
    } else { std::cerr << "    [SKIP] walkAnim is NULL\n"; }
    std::cout << "  [DEBUG] Loading Run (ptr=" << (void*)runAnim.get() << ")...\n"; fflush(stdout);
    if (runAnim) {
        try {
            hybrid->LoadLocomotionAnimation("Run", runAnim);
            std::cout << "    Run loaded: duration=" << runAnim->duration << "s, bones=" << runAnim->boneAnimations.size() << "\n";
        } catch (const std::exception& e) {
            std::cerr << "    [EXCEPTION in LoadLocomotionAnimation Run]: " << e.what() << "\n";
            throw;
        }
    } else { std::cerr << "    [SKIP] runAnim is NULL\n"; }
    
    // Also load crouch animations into locomotion for smooth blending
    std::cout << "  [DEBUG] Loading Crouch (locomotion)...\n"; fflush(stdout);
    if (crouchAnim) {
        hybrid->LoadLocomotionAnimation("Crouch", crouchAnim);
        std::cout << "    Crouch loaded: duration=" << crouchAnim->duration << "s\n";
    }
    std::cout << "  [DEBUG] Loading CrouchWalk (locomotion)...\n"; fflush(stdout);
    if (crouchWalkAnim) {
        hybrid->LoadLocomotionAnimation("CrouchWalk", crouchWalkAnim);
        std::cout << "    CrouchWalk loaded: duration=" << crouchWalkAnim->duration << "s\n";
    }

    // Load state animations (for FSM states) - skip NULL
    std::cout << "  [DEBUG] Loading Jump...\n"; fflush(stdout);
    if (jumpAnim) hybrid->LoadStateAnimation(HybridState::JUMP, "Jump", jumpAnim);
    std::cout << "  [DEBUG] Loading Fall...\n"; fflush(stdout);
    if (fallAnim) hybrid->LoadStateAnimation(HybridState::FALL, "Fall", fallAnim);
    else std::cerr << "[HybridMMFSM] WARNING: Fall animation is NULL, skipping\n";
    std::cout << "  [DEBUG] Loading Crouch...\n"; fflush(stdout);
    if (crouchAnim) hybrid->LoadStateAnimation(HybridState::CROUCH, "Crouch", crouchAnim);
    std::cout << "  [DEBUG] Loading CrouchWalk...\n"; fflush(stdout);
    if (crouchWalkAnim) hybrid->LoadStateAnimation(HybridState::CROUCH_WALK, "CrouchWalk", crouchWalkAnim);

    // Build databases
    std::cout << "  [DEBUG] Building databases...\n"; fflush(stdout);
    hybrid->BuildDatabases();
    std::cout << "  [DEBUG] Databases built\n"; fflush(stdout);
    
    // Debug: Check motion matcher database
    std::cout << "\n=== MOTION MATCHER DATABASE STATS ===\n";
    std::cout << "Total poses in MM database: " << hybrid->GetMotionMatcherDatabaseSize() << "\n";
    std::cout << "=====================================\n\n";

    // ============================================================
    // ADD FSM TRANSITIONS (CRITICAL - enables state changes)
    // ============================================================
    std::cout << "\n=== ADDING HYBRID FSM TRANSITIONS ===\n";
    
    // Jump transition (LOCOMOTION -> JUMP)
    hybrid->AddTransition(HybridState::LOCOMOTION, HybridState::JUMP, 0.1f,
        [hybridPtr = hybrid]() { 
            return hybridPtr->GetCharacterState().jump && hybridPtr->GetCharacterState().grounded; 
        }, true);
    std::cout << "  Added: LOCOMOTION -> JUMP (on jump press, grounded)\n";
    
    // Fall transition (LOCOMOTION -> FALL)
    hybrid->AddTransition(HybridState::LOCOMOTION, HybridState::FALL, 0.15f,
        [hybridPtr = hybrid]() { 
            return !hybridPtr->GetCharacterState().grounded && hybridPtr->GetCharacterState().velocity.y < -0.5f; 
        }, true);
    std::cout << "  Added: LOCOMOTION -> FALL (when airborne, falling)\n";
    
    // Crouch transition (LOCOMOTION -> CROUCH)
    hybrid->AddTransition(HybridState::LOCOMOTION, HybridState::CROUCH, 0.2f,
        [hybridPtr = hybrid]() { 
            return hybridPtr->GetCharacterState().crouch && hybridPtr->GetCharacterState().grounded; 
        }, true);
    std::cout << "  Added: LOCOMOTION -> CROUCH (on crouch press)\n";
    
    // Return to locomotion from JUMP (when landed)
    hybrid->AddTransition(HybridState::JUMP, HybridState::LOCOMOTION, 0.1f,
        [hybridPtr = hybrid]() { 
            return hybridPtr->GetCharacterState().grounded; 
        }, true);
    std::cout << "  Added: JUMP -> LOCOMOTION (when landed)\n";
    
    // Return to locomotion from FALL (when landed)
    hybrid->AddTransition(HybridState::FALL, HybridState::LOCOMOTION, 0.1f,
        [hybridPtr = hybrid]() { 
            return hybridPtr->GetCharacterState().grounded; 
        }, true);
    std::cout << "  Added: FALL -> LOCOMOTION (when landed)\n";
    
    // Return to locomotion from CROUCH (when stand up)
    hybrid->AddTransition(HybridState::CROUCH, HybridState::LOCOMOTION, 0.2f,
        [hybridPtr = hybrid]() { 
            return !hybridPtr->GetCharacterState().crouch && hybridPtr->GetCharacterState().grounded; 
        }, true);
    std::cout << "  Added: CROUCH -> LOCOMOTION (when stand up)\n";
    
    std::cout << "FSM transitions configured!\n";

    std::cout << "[HybridMMFSM] Initialized - MM for locomotion, FSM for states\n";
    std::cout << "[HybridMMFSM] Note: Transitions handled in game loop via state checks\n";

    // Initialize demo recorder
    std::cout << "\nInitializing Demo Recorder...\n";
    demoRecorder = new DemoRecorder();
    demoRecorder->initialize();
    std::cout << "Demo Recorder ready - Press F6 to toggle recording/playback\n";
    ...
    auto printAnimInfo = [](std::shared_ptr<Animation> anim, const std::string& name) {
        if (anim) {
    ...
    // CRITICAL: Play idle animation at startup to prevent T-pose
    if (idleAnim && animator) {
        std::cout << "\n[STARTUP] Playing idle animation to prevent T-pose...\n";
        animator->Play(idleAnim.get());
        animator->SetCurrentTime(0.0f);
        animator->Update(0.0f);  // Initial bone matrix calculation
        std::cout << "[STARTUP] Idle animation playing\n";
    }
    ...
    std::cout << "\n=== READY ===\n\n";
    std::cout << "Character: Bot (ANIMATED)\n";
    ...
    std::cout << "Press G for foot IK status\n";
    std::cout << "Press H to print motion matching debug\n";
    std::cout << "Press M to view loaded models (Bot is playable character)\n";
    std::cout << "Press F6 to toggle demo recording/playback\n";
    ...
    while (running && !glfwWindowShouldClose(window)) {
        ...
        // Update demo recorder (handles playback/recording)
        if (demoRecorder) {
            demoRecorder->update(dt);
            
            // If in playback mode, update camera from demo path
            if (demoRecorder->isPlaying()) {
                demoRecorder->updateCamera(camera);
            }
        }
        ...
        // Debug: print key state every 60 frames
        static int keyFrame = 0;
        if (++keyFrame % 60 == 0) {
            std::cout << "[KEYS] W=" << (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS ? "1" : "0")
                      << " S=" << (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS ? "1" : "0")
                      << " A=" << (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS ? "1" : "0")
                      << " D=" << (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS ? "1" : "0")
        ...
        // DEBUG: Print input state and MM query every frame when moving
        static glm::vec2 prevMoveDir(0.0f);
        if (moveDir != prevMoveDir) {
            std::cout << "\n[INPUT CHANGE] Dir=(" << moveDir.x << "," << moveDir.y 
                      << ") Mag=" << charInput.moveMagnitude 
                      << " | MM Query: speed=" << charInput.moveMagnitude * 2.0f << "\n";
            prevMoveDir = moveDir;
        }
        ...
        // CRITICAL FIX: Only trigger jump ONCE per key press
        // AND don't trigger if already in jump animation
        charInput.jump = jumpKeyPressed && !lastJump;
        ...
        // Track jump animation state for grounded calculation
        static bool inJumpAnimation = false;
        static float jumpAnimationTimer = 0.0f;
        static float jumpVelocity = 0.0f;
        
        // CRITICAL: Don't allow jump trigger if already in jump animation
        // This prevents re-triggering and timer reset
        if (charInput.jump && wasGrounded && !inJumpAnimation) {
            jumpVelocity = 5.0f;  // Initial jump impulse
            charInput.verticalVelocity = jumpVelocity;
            inJumpAnimation = true;  // Mark that we're in jump animation
            jumpAnimationTimer = 0.0f;
            charInput.grounded = true;  // FSM needs to see grounded=true for jump trigger
        } else if (!wasGrounded || inJumpAnimation) {
            jumpVelocity -= 12.0f * dt;  // Gravity
            charInput.verticalVelocity = jumpVelocity;
            
            // Keep grounded=false during jump animation (even if terrain says grounded)
            if (inJumpAnimation) {
                jumpAnimationTimer += dt;
                // Jump animation typically lasts ~0.8-1.2s, keep airborne for at least 0.5s
                if (jumpAnimationTimer < 0.5f) {
                    charInput.grounded = false;
                } else if (wasGrounded) {
                    // Landed after minimum jump duration
                    inJumpAnimation = false;
                    charInput.grounded = true;
                    jumpVelocity = 0.0f;
                }
            } else {
                charInput.grounded = false;  // Normal falling
            }

            if (jumpVelocity <= 0.0f && characterPos.y <= terrainHeight + 0.01f && !inJumpAnimation) {
                charInput.grounded = true;
                jumpVelocity = 0.0f;
            }
        } else {
        ...
        if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS && !lastH && hybrid) {
            std::cout << "\n=== HYBRID MM+FSM DEBUG ===\n";
            std::cout << hybrid->GetDebugInfo();
        ...
        // Toggle demo recording/playback (F6)
        static bool lastF6 = false;
        if (glfwGetKey(window, GLFW_KEY_F6) == GLFW_PRESS && !lastF6 && demoRecorder) {
            demoRecorder->toggleMode();
        }
        lastF6 = glfwGetKey(window, GLFW_KEY_F6) == GLFW_PRESS;

        // Switch models (M key) - VIEW MODE only, doesn't change playable character
        ...
        if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS && !lastM) {
            ...
            if (!modelNames.empty()) {
                ...
                std::string newName = modelNames[currentModelIndex];
                std::cout << ">>> Model view: " << newName << " (Bot is still playable character)\n";
                std::cout << "    Note: Only Bot has animations. Other models are static props.\n";
            }
        }
        lastM = glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS;

        // ============================================================
        // EXTRACT ROOT MOTION (needed for hybrid state velocity AND movement)
        // ============================================================
        glm::vec3 rootMotionFromAnim(0.0f);  // Root motion from animation (for velocity calc)
        glm::vec3 rootMotionDelta(0.0f);     // Root motion for position update
        
        if (animator) {
            glm::vec3 rootMotion = animator->ConsumeRootMotion();
            rootMotionFromAnim = rootMotion;
            
            // Calculate movement delta (will be applied later)
            if (charInput.moveMagnitude > 0.01f && moveDirection != glm::vec3(0.0f)) {
                rootMotionDelta = moveDirection * glm::length(rootMotion);
            }
        }

        // ============================================================
        // BUILD MODEL MATRIX (needed for IK and rendering)
        // ============================================================
        ...
        // ============================================================
        // UPDATE HYBRID MM+FSM (DRIVES ANIMATOR)
        // ============================================================
        if (hybrid) {
            // Build hybrid state input from character input
            HybridMMFSMState hybridState;
            hybridState.moveDirection = charInput.moveDirection;
            hybridState.moveMagnitude = charInput.moveMagnitude;
            hybridState.jump = charInput.jump;
            hybridState.crouch = charInput.crouch;
            hybridState.sprint = charInput.sprint;
            hybridState.position = characterPos;

            // CRITICAL: Use TARGET speed based on INPUT, scaled to match EXTRACTED speeds
            // Due to animation duration truncation, extracted speeds are exaggerated:
            // - Idle: ~0.03 m/s (correct, no root motion)
            // - Walk: ~5.4 m/s (exaggerated 10x due to 29s->1.2s truncation)
            // - Run: ~17.7 m/s (exaggerated 18x due to 16s->0.9s truncation)
            float targetSpeed = 0.0f;
            if (charInput.moveMagnitude > 0.1f) {
                // Scale input to match Walk's extracted speed range
                // Input 0-1 → Speed 0.0-5.4 m/s
                targetSpeed = charInput.moveMagnitude * 5.4f;
            }
            
            // Build velocity vector in move direction
            glm::vec3 targetVelocity(0.0f);
            if (targetSpeed > 0.001f && moveDirection != glm::vec3(0.0f)) {
                targetVelocity = glm::normalize(moveDirection) * targetSpeed;
            }
            
            // Use target velocity for MM query (not previous frame's root motion)
            hybridState.velocity = targetVelocity;
            
            // Debug: print velocity being sent to hybrid
            static int debugFrame = 0;
            if (++debugFrame % 30 == 0) {
                std::cout << "[Hybrid Input] velocity=(" << hybridState.velocity.x << "," << hybridState.velocity.z 
                          << ") targetSpeed=" << targetSpeed << " moveDir=" << charInput.moveMagnitude << "\n";
            }

            hybridState.rotation = rotationAngle;
            hybridState.grounded = charInput.grounded;

            // Update hybrid system - this drives the animator internally
            hybrid->Update(dt, hybridState);

            // ============================================================
            // FOOT IK MUST RUN BEFORE animator->Update()
            // ============================================================
            // CRITICAL: UpdateFootIK sets ikOffsets, which are used by
            // animator->Update() when calculating bone matrices.
            // If we call UpdateFootIK AFTER Update(), the IK is 1 frame late!
            if (animator) {
                ...
                if (terrain) {
                    ...
                    if (std::isfinite(terrainHeight)) {
                        ...
                        ikSettings.floorHeight = terrainHeight;
                        ikSettings.enabled = true;
                        animator->SetFootIKSettings(ikSettings);
                        static int ikDebugFrame = 0;
                        if (++ikDebugFrame % 30 == 0) std::cout << "[FootIK Setup] terrainHeight=" << terrainHeight << " at pos=(" << characterPos.x << "," << characterPos.z << ")\n";
                    } else {
                        static int ikDebugFrame = 0;
                        if (++ikDebugFrame % 30 == 0) std::cout << "[FootIK Setup] terrainHeight is NaN/Inf!\n";
                    }
                }
            ...
            // CRITICAL: Update animator to calculate bone matrices (with IK applied)
            if (animator) {
                animator->Update(dt);
            }

            // Debug: Print state and MM info every 2 seconds
            static float hybridDebugTimer = 0.0f;
            hybridDebugTimer += dt;
            if (hybridDebugTimer > 2.0f) {
                std::cout << "\n[Hybrid] State=" << HybridStateToString(hybrid->GetCurrentState())
                          << " | MoveMag=" << charInput.moveMagnitude
                          << " | Grounded=" << charInput.grounded << "\n";
                std::cout << hybrid->GetMotionMatcherDebug();
                hybridDebugTimer = 0.0f;
            }
        }

        // ============================================================
        // UPDATE ANIMATION FSM (FIXED)
        // ============================================================
        if (fsm) {
            // Update FSM with character input
            fsm->update(dt, charInput);

            // Debug: Print state every 2 seconds
            static float fsmDebugTimer = 0.0f;
            fsmDebugTimer += dt;
            if (fsmDebugTimer > 2.0f) {
                std::cout << "[FSM] State=" << AnimationStateToString(fsm->getCurrentState())
                          << " Speed=" << charInput.moveMagnitude
                          << " Grounded=" << charInput.grounded
                          << " Blend=" << fsm->getCurrentBlendWeight() << "\n";
                fsmDebugTimer = 0.0f;
            }
        }

        // Apply root motion to character position (calculated above)
        if (glm::length(rootMotionDelta) > 0.0001f) {
            characterPos += rootMotionDelta;
        }

        // FALLBACK: If no root motion but moving, use direct movement
        if (animator && glm::length(rootMotionDelta) < 0.0001f && charInput.moveMagnitude > 0.01f) {
            float walkSpeed = 5.0f;  // units per second
            float runSpeed = 10.0f;  // units per second
            float speed = charInput.sprint ? runSpeed : walkSpeed;
            float motionMagnitude = glm::length(rootMotionFromAnim);
            
            // DEBUG: Print root motion info
            static float debugTimer = 0.0f;
            debugTimer += dt;
            if (debugTimer > 2.0f) {
                std::cout << "[RootMotion] Mag=" << motionMagnitude
                          << " Dir=(" << moveDirection.x << "," << moveDirection.y << "," << moveDirection.z << ")"
                          << " MoveMag=" << charInput.moveMagnitude
                          << (motionMagnitude > 0.001f ? " ✓" : " ✗ NO MOTION - using fallback") << "\n";
                debugTimer = 0.0f;
            }

            rootMotionDelta = moveDirection * speed * dt;
            characterPos += rootMotionDelta;

            // Debug output for fallback movement
            static float fallbackDebugTimer = 0.0f;
            fallbackDebugTimer += dt;
            if (fallbackDebugTimer > 2.0f) {
                std::cout << "[FALLBACK] Direct movement: speed=" << speed
                          << " delta=" << glm::length(rootMotionDelta) << "\n";
                fallbackDebugTimer = 0.0f;
            }
        }
        ...
        if (vegetation && terrain) {
            ...
            // Place special props (Bear, Datsun) with proper physics and separation
            static bool propsPlaced = false;
            if (!propsPlaced && worldObjects && terrain) {
                std::cout << "\n=== Placing Special Props (Bear, Datsun) ===\n";
                
                // Find flatter ground - search areas closer to spawn where terrain is lower
                // Bear: Search 20-60m from origin in positive X
                // Datsun: Search -60 to -20m from origin in negative X
                glm::vec3 bearPos = findFlatGround(terrain, 20.0f, 60.0f, 15.0f);
                glm::vec3 datsunPos = findFlatGround(terrain, -60.0f, -20.0f, 15.0f);
                
                // Place Bear model with gravity
                Model* bearModel = modelManager.get("Bear");
                if (bearModel) {
                    ModelPlacement bearConfig;
                    bearConfig.model = bearModel;
                    bearConfig.position = bearPos;
                    bearConfig.useGravity = true;
                    bearConfig.randomRotation = true;
                    bearConfig.scale = 1.0f;
                    
                    glm::mat4 bearMat = ModelPlacer::placeModel(bearConfig, terrain);
                    (void)bearMat;  // Suppress unused warning
                    std::cout << "[World] 🐻 Bear placed at (" << bearConfig.position.x 
                              << ", " << bearConfig.position.y << ", " << bearConfig.position.z 
                              << ") - terrain height: " << terrain->getHeightAt(bearConfig.position.x, bearConfig.position.z) << "\n";
                }
                
                // Place Datsun car with gravity
                Model* datsunModel = modelManager.get("Datsun");
                if (datsunModel) {
                    ModelPlacement datsunConfig;
                    datsunConfig.model = datsunModel;
                    datsunConfig.position = datsunPos;
                    datsunConfig.useGravity = true;
                    datsunConfig.rotation = 1.57f;  // Face sideways (90 degrees)
                    datsunConfig.scale = 1.0f;
                    
                    glm::mat4 datsunMat = ModelPlacer::placeModel(datsunConfig, terrain);
                    (void)datsunMat;  // Suppress unused warning
                    std::cout << "[World] 🚗 Datsun placed at (" << datsunConfig.position.x 
                              << ", " << datsunConfig.position.y << ", " << datsunConfig.position.z 
                              << ") - terrain height: " << terrain->getHeightAt(datsunConfig.position.x, datsunConfig.position.z) << "\n";
                }
                
                propsPlaced = true;
                std::cout << "[World] Special props placed with physics!\n";
            }
    ...
    // Cleanup - shared_ptr animations clean up automatically
    modelManager.clear();  // Clean up all loaded models
    if (animator) delete animator;
    // walkAnim, idleAnim, etc. are shared_ptr - automatic cleanup
    if (grassModel) delete grassModel;
    if (terrain) delete terrain;
    if (vegetation) delete vegetation;
    if (worldObjects) delete worldObjects;
    if (demoRecorder) delete demoRecorder;
    glfwTerminate();
