...
int main()
{
    ...
    // Helper lambda to load animation - returns shared_ptr
    ...
    std::cout << "  [DEBUG] Loading Idle...\n"; fflush(stdout);
    if (idleAnim) hybrid->LoadLocomotionAnimation("Idle", idleAnim);
    std::cout << "  [DEBUG] Loading Walk...\n"; fflush(stdout);
    if (walkAnim) hybrid->LoadLocomotionAnimation("Walk", walkAnim);
    std::cout << "  [DEBUG] Loading Run...\n"; fflush(stdout);
    if (runAnim) hybrid->LoadLocomotionAnimation("Run", runAnim);
    ...
    while (running && !glfwWindowShouldClose(window)) {
        ...
        static float inputDebugTimer = 0.0f;
        inputDebugTimer += dt;
        if (inputDebugTimer > 1.0f) {
            std::cout << "[INPUT] W=" << (moveDir.y > 0 ? "1" : "0") 
                      << " S=" << (moveDir.y < 0 ? "1" : "0")
                      << " A=" << (moveDir.x > 0 ? "1" : "0")
                      << " D=" << (moveDir.x < 0 ? "1" : "0")
                      << " Mag=" << charInput.moveMagnitude << "\n";
            inputDebugTimer = 0.0f;
        }
        ...
        if (animator) {
            ...
            // DEBUG: Print root motion info (even if zero)
            ...
            if (debugTimer > 2.0f) {
                std::cout << "[RootMotion] Mag=" << motionMagnitude
                          ...
                          << (motionMagnitude > 0.001f ? " ✓" : " ✗ NO MOTION") << "\n";
                debugTimer = 0.0f;
            }

            // CRITICAL FIX: Apply root motion DIRECTLY (no multiplier) for accurate foot placement
            // Root motion magnitude IS the movement distance - don't scale it down
            if (charInput.moveMagnitude > 0.01f && moveDirection != glm::vec3(0.0f)) {
                float movementMultiplier = 1.0f;  // DIRECT application - no scaling!
                rootMotionDelta = moveDirection * motionMagnitude * movementMultiplier;
                characterPos += rootMotionDelta;
            }

            // FALLBACK: If no root motion, use direct movement (for locked-root animations)
            if (motionMagnitude < 0.001f && charInput.moveMagnitude > 0.01f) {
            ...
            // CRITICAL FIX: Enable foot IK IMMEDIATELY with aggressive settings
            if (animator && !animator->footIKSettings.enabled) {
                Animator::FootIKSettings ikSettings;
                ikSettings.enabled = true;
                ikSettings.floorHeight = characterPos.y;
                ikSettings.ikStrength = 1.0f;
                ikSettings.footLockBlend = 0.95f;  // VERY high blend to lock feet when planted
                ikSettings.maxIKDistance = 0.05f;  // TIGHT limit to prevent sliding
                ikSettings.footLockReleaseSpeed = 3.0f;  // Fast release when foot lifts
                animator->SetFootIKSettings(ikSettings);
                std::cout << "[FootIK] Enabled with AGGRESSIVE anti-footskate settings\n";
            }
