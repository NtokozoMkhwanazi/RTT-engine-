...
int main()
{
    ...
    auto LoadAnim = [&](const std::string &path)->Animation{
        ...
        return AssimpAnimationLoader::LoadAnimation(scene,scene->mAnimations[0]);
    };
    ...
    floorBody->mass = 0.0f; floorBody->isStatic=true;
    physicsWorld.addBody(floorBody);

    SimpleMesh floorMesh; floorMesh.createCube(floorBody->scale.x,floorBody->scale.y,floorBody->scale.z);

    auto cubeBody = std::make_shared<RigidBody>();
    cubeBody->scale = glm::vec3(0.5f);
    cubeBody->position = glm::vec3(2.0f,5.0f,0.0f);
    cubeBody->mass = 1.0f; cubeBody->isModel=true; cubeBody->isStatic=false;
    physicsWorld.addBody(cubeBody);

    SimpleMesh cubeMesh; cubeMesh.createCube(cubeBody->scale.x,cubeBody->scale.y,cubeBody->scale.z);
    ...
    playerBody->mass=1.0f; playerBody->isModel=true; playerBody->isStatic=false;
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        if(hipsIdx==-1) hipsIdx=skeleton.rootBoneIndex;
      ...
      //  glm::mat4 rotationFix = glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        ...
        adjustedPosition.y += 10.0f * modelScale; // Compensate for the vertical flip
        
        glm::mat4 modelMat = glm::translate(glm::mat4(1.0f), adjustedPosition)
        ...
        auto processFoot = [&](int bone,FootLock &lock){
            if(bone<0 || bone>=(int)animator.GetFinalBoneMatrices().size()) return;
        ...
        if (debugCounter < 5) { // Only print first few frames
